...
define( 'ABSPATH', '/tmp/' );
define( 'SVC_VERSION', '0.1.0' );
define( 'SVC_PLUGIN_DIR', dirname( __DIR__ ) . '/swiss-volley-connector/' );
define( 'SVC_PLUGIN_URL', 'https://example.test/wp-content/plugins/swiss-volley-connector/' );
...
require SVC_PLUGIN_DIR . 'includes/class-svc-logger.php';
require SVC_PLUGIN_DIR . 'includes/class-svc-cache.php';
require SVC_PLUGIN_DIR . 'includes/class-svc-api.php';
require SVC_PLUGIN_DIR . 'includes/class-svc-teams.php';
require SVC_PLUGIN_DIR . 'includes/class-svc-data.php';
require SVC_PLUGIN_DIR . 'includes/class-svc-renderer.php';
require SVC_PLUGIN_DIR . 'includes/class-svc-plugin.php';
require SVC_PLUGIN_DIR . 'includes/class-svc-shortcodes.php';
...
function reset_state( array $settings = array() ): void {
	...
	$GLOBALS['http_calls']    = 0;
	$GLOBALS['wp_options']['svc_settings'] = array_merge( SVC_Plugin::default_settings(), array( 'api_key' => 'TESTKEY' ), $settings );
	// Runtime-Cache von SVC_Data zurücksetzen.
	$ref  = new ReflectionClass( 'SVC_Data' );
...
echo "== Swiss Volley Connector – Testszenarien ==\n\n";
...
reset_state(); mock_ok();
SVC_Data::raw_games(); // füllt Cache + Stale.
reset_state();         // Runtime + Transients weg …
$GLOBALS['wp_options']['svc_stale_games'] = array( 'saved' => time() - 3600, 'data' => fixture_games() ); // … Stale bleibt.
$GLOBALS['http_mock'] = fn() => new WP_Error( 'down', 'down' );
$games = SVC_Data::games_for_team( 101, 'upcoming', 5 );
check( '03c Stale-Fallback liefert zuletzt bekannte Daten', is_array( $games ) && count( $games ) === 2 && SVC_Data::served_stale() );
$html = SVC_Renderer::render_games( $games );
...
check( '05 Club nicht gefunden bei leeren Daten', null === SVC_Data::detect_own_club( array() ) );

// 6. Team gefunden: Ableitung der Teams.
reset_state(); mock_ok();
$raw   = SVC_Data::raw_games();
$teams = SVC_Data::derive_teams( $raw, 'VP1' );
...
SVC_Teams::merge_derived( $teams );
$stored = SVC_Teams::all();
$stored[101]['alias'] = 'herren-1';
SVC_Teams::save( $stored );

// 7. Team ohne Spiele (unbekannte/inaktive Team-ID).
$empty = SVC_Data::games_for_team( 99999, 'upcoming', 5 );
check( '07 Team ohne Spiele: leere Liste, keine Fehler', is_array( $empty ) && 0 === count( $empty ) );
$html = SVC_Renderer::render_games( $empty );
check( '07b Leermeldung wird angezeigt', str_contains( $html, 'svc-empty' ) );

// 8. Kommende Spiele vorhanden + aufsteigende Sortierung + Cup enthalten.
$up = SVC_Data::games_for_team( 101, 'upcoming', 10 );
...
$res = SVC_Data::games_for_team( 101, 'played', 10 );
...
$rank = SVC_Data::ranking_for_team( 101 );
...
// 11. Resultat 3:0.
$d1 = SVC_Data::games_for_team( 201, 'played', 5 );
...
$html = SVC_Renderer::render_games( array( $res[0] ) );
...
$up2 = SVC_Data::games_for_team( 101, 'upcoming', 10 );
...
$ref = new ReflectionClass( 'SVC_Data' ); $p = $ref->getProperty( 'runtime' ); $p->setAccessible( true ); $p->setValue( null, array() );
SVC_Data::raw_games();
...
reset_state(); mock_ok();
SVC_Teams::merge_derived( SVC_Data::derive_teams( SVC_Data::raw_games(), 'VP1' ) );
$stored = SVC_Teams::all(); $stored[101]['alias'] = 'herren-1'; SVC_Teams::save( $stored );
$GLOBALS['wp_options']['svc_settings']['club_id'] = 'VP1';

$html = SVC_Shortcodes::team( array( 'team' => 'herren-1', 'limit' => 5 ) );
check( 'S1 Alias-Auflösung + kombinierte Teamansicht', str_contains( $html, 'svc-team-view' ) && str_contains( $html, 'chste Spiele' ) && str_contains( $html, 'svc-ranking' ) );
check( 'S2 Eigenes Team mit svc-own-team hervorgehoben', str_contains( $html, 'svc-own-team' ) );

$html = SVC_Shortcodes::club_games( array( 'limit' => 10 ) );
check( 'S3 Vereinsweite kommende Spiele über alle Teams', substr_count( $html, 'svc-game ' ) === 2 || substr_count( $html, '<article' ) === 2 );

$html = SVC_Shortcodes::club_results( array( 'limit' => 10 ) );
check( 'S4 Vereinsweite Resultate (3 Spiele, absteigend)', 3 === substr_count( $html, '<article' ) && str_contains( $html, 'svc-result-sets' ) );

$html = SVC_Shortcodes::games( array( 'team' => 'gibts-nicht' ) );
check( 'S5 Unbekanntes Team: verständliche Meldung', str_contains( $html, 'nicht gefunden' ) );

// Team-Filter für Club-Ansichten.
$stored = SVC_Teams::all(); $stored[201]['in_club'] = false; SVC_Teams::save( $stored );
$html = SVC_Shortcodes::club_results( array( 'limit' => 10 ) );
...
$up3  = SVC_Data::games_for_team( 101, 'upcoming', 5 );
$html = SVC_Renderer::render_games( $up3 );
...
SVC_Teams::save( $stored );
$up  = SVC_Data::games_for_team( 101, 'upcoming', 10 );
$cg  = SVC_Data::games_for_club( 'upcoming', 10 );
...
$d = SVC_Data::games_for_team( 201, 'played', 5 );
check( 'L4 Anderes Team unverändert', 'Damen 3. Liga' === $d[0]['league'] );
SVC_Teams::merge_derived( SVC_Data::derive_teams( SVC_Data::raw_games(), 'VP1' ) );
check( 'L5 Neuladen der Teams erhält eigene Bezeichnung', 'Herren 2. Liga Region GL/GR/SG' === SVC_Teams::league_label( 101 ) );

// L6: Renderer gibt die Bezeichnung escaped aus.
$html = SVC_Renderer::render_games( SVC_Data::games_for_team( 101, 'upcoming', 1 ) );
...
$cg = SVC_Data::games_for_club( 'played', 10 );
...
// N6: Neuladen erhält den eigenen Teamnamen; Anzeige inkl. svc-own-team im HTML.
SVC_Teams::merge_derived( SVC_Data::derive_teams( SVC_Data::raw_games(), 'VP1' ) );
check( 'N6 Neuladen erhält eigenen Teamnamen', 'Herren 1' === SVC_Teams::name_label( 101 ) );
$html = SVC_Renderer::render_games( SVC_Data::games_for_team( 101, 'upcoming', 1 ) );
check( 'N7 Eigener Name im HTML mit svc-own-team', str_contains( $html, 'Herren 1' ) && str_contains( $html, 'svc-own-team' ) );

// Liga als Überschrift.
$html = SVC_Shortcodes::club_games( array( 'limit' => 10 ) );
check( 'G1 Club-Ansicht: Liga als Überschrift (h3)', str_contains( $html, '<h3 class="svc-game-league">' ) );
check( 'G2 Club-Ansicht: Liga nicht doppelt in Meta-Zeile', ! str_contains( $html, 'svc-league"' ) );
check( 'G3 Eigene Liga-Bezeichnung in der Überschrift', str_contains( $html, 'Herren 2. Liga Region GL/GR/SG</h3>' ) );

$GLOBALS['wp_options']['svc_settings']['league_display'] = 'meta';
$html = SVC_Shortcodes::games( array( 'team' => '101' ) );
check( 'G4 Teamansicht folgt globaler Einstellung (meta)', str_contains( $html, 'svc-league"' ) && ! str_contains( $html, 'svc-game-league' ) );
$GLOBALS['wp_options']['svc_settings']['league_display'] = 'heading';

$html = SVC_Shortcodes::games( array( 'team' => '101', 'league' => 'heading' ) );
check( 'G5 league="heading" auch für Team-Shortcodes', str_contains( $html, 'svc-game-league' ) );

$html = SVC_Shortcodes::club_results( array( 'limit' => 10, 'league' => 'none' ) );
check( 'G6 league="none" blendet Liga aus', ! str_contains( $html, 'svc-game-league' ) && ! str_contains( $html, 'svc-league"' ) );
...
$html = SVC_Renderer::render_games( SVC_Data::games_for_team( 101, 'upcoming', 10 ) );
check( 'U1 Heimteam verlinkt', str_contains( $html, '<a class="svc-team svc-team-home svc-own-team svc-team-link" href="https://www.volleypizol.org/herren-1/">' ) );
check( 'U2 Gegner nicht verlinkt', str_contains( $html, '<span class="svc-team svc-team-away">Volley N' ) );

// U3: Auswärtsspiel → Auswärtsteam verlinkt.
$html = SVC_Renderer::render_games( SVC_Data::games_for_team( 101, 'played', 10 ) );
check( 'U3 Auswärtsteam verlinkt', str_contains( $html, 'svc-team-away svc-own-team svc-team-link' ) );

// U4: Rangliste → eigene Zeile verlinkt, Gegner nicht.
$html = SVC_Renderer::render_ranking( SVC_Data::ranking_for_team( 101 ), 101 );
check( 'U4 Rangliste: eigenes Team verlinkt', str_contains( $html, '<a class="svc-team-link" href="https://www.volleypizol.org/herren-1/">' ) && ! str_contains( $html, 'Jona</a>' ) );

// U5: Ohne Link bleibt alles wie bisher (Team 201).
$html = SVC_Renderer::render_games( SVC_Data::games_for_team( 201, 'played', 5 ) );
check( 'U5 Ohne Link: Span statt Anker', ! str_contains( $html, 'svc-team-link' ) );

// U6: Neuladen der Teams erhält den Link.
SVC_Teams::merge_derived( SVC_Data::derive_teams( SVC_Data::raw_games(), 'VP1' ) );
check( 'U6 Neuladen erhält Team-Link', 'https://www.volleypizol.org/herren-1/' === SVC_Teams::page_url( 101 ) );
...
$html = SVC_Shortcodes::club_games( array( 'limit' => 10 ) );
check( 'U8 Club-Ansicht mit Team-Link', str_contains( $html, 'svc-team-link' ) );

// Globale Liga-Darstellung.
// K1: Standard 'heading' gilt jetzt auch für Team-Shortcodes.
$html = SVC_Shortcodes::games( array( 'team' => '101' ) );
check( 'K1 Global heading: Team-Liste mit Überschrift', str_contains( $html, 'svc-game-league' ) );

// K2: Global 'meta' → Club-Liste klein in der Detailzeile.
$GLOBALS['wp_options']['svc_settings']['league_display'] = 'meta';
$html = SVC_Shortcodes::club_games( array( 'limit' => 10 ) );
check( 'K2 Global meta: Club-Liste in Detailzeile', str_contains( $html, 'svc-league"' ) && ! str_contains( $html, 'svc-game-league' ) );

// K3: Shortcode-Attribut übersteuert die Einstellung.
$html = SVC_Shortcodes::club_games( array( 'limit' => 10, 'league' => 'heading' ) );
check( 'K3 Attribut übersteuert Einstellung', str_contains( $html, 'svc-game-league' ) );

// K4: Ungültiger Einstellungswert fällt auf heading zurück.
$GLOBALS['wp_options']['svc_settings']['league_display'] = 'quatsch';
$html = SVC_Shortcodes::games( array( 'team' => '101' ) );
check( 'K4 Ungültiger Wert: Fallback heading', str_contains( $html, 'svc-game-league' ) );
$GLOBALS['wp_options']['svc_settings']['league_display'] = 'heading';

// Gruppierung.
// P1: Nach Liga gruppiert – Gruppen-Überschriften, Liga pro Karte ausgeblendet.
$html = SVC_Shortcodes::club_games( array( 'limit' => 10, 'group_by' => 'league' ) );
check( 'P1 Gruppierung nach Liga: Sections + Überschriften', substr_count( $html, '<section class="svc-game-group">' ) === 2 && str_contains( $html, '<h3 class="svc-group-heading">' ) );
check( 'P2 Liga pro Karte ausgeblendet (nicht doppelt)', ! str_contains( $html, 'svc-game-league' ) && ! str_contains( $html, 'svc-league"' ) );
...
$html = SVC_Shortcodes::club_games( array( 'limit' => 10, 'group_by' => 'league', 'league' => 'heading' ) );
check( 'P4 Explizites league="heading" bleibt in Gruppen erhalten', str_contains( $html, 'svc-game-league' ) );

// P5: Nach Team gruppiert – Überschrift = eigener Teamname, Liga weiterhin da.
$html = SVC_Shortcodes::club_results( array( 'limit' => 10, 'group_by' => 'team' ) );
check( 'P5 Gruppierung nach Team mit eigenem Teamnamen', str_contains( $html, '<h3 class="svc-group-heading">Herren 1</h3>' ) && str_contains( $html, 'svc-game-league' ) );
check( 'P6 Zwei Teamgruppen (Herren 1, Damen 1)', 2 === substr_count( $html, 'svc-group-heading' ) );

// P7: 'liga' als deutsches Synonym.
$html = SVC_Shortcodes::club_games( array( 'limit' => 10, 'group_by' => 'liga' ) );
check( 'P7 group_by="liga" wird akzeptiert', str_contains( $html, 'svc-group-heading' ) );

// P8: Ohne group_by unverändert (keine Sections).
$html = SVC_Shortcodes::club_games( array( 'limit' => 10 ) );
check( 'P8 Ohne group_by keine Gruppen', ! str_contains( $html, 'svc-game-group' ) );

// P9: Auch für Team-Shortcodes (Meisterschaft/Cup getrennt).
$html = SVC_Shortcodes::games( array( 'team' => '101', 'group_by' => 'league' ) );
check( 'P9 Team-Shortcode nach Liga gruppiert (Cup separat)', 2 === substr_count( $html, 'svc-group-heading' ) );

// Interaktiver Umschalter.
// W1: Einstellung aus (Standard) → kein Umschalter.
$html = SVC_Shortcodes::club_results( array( 'limit' => 10 ) );
check( 'W1 Einstellung aus: kein Umschalter', ! str_contains( $html, 'svc-switcher' ) );

// W2: Einstellung ein → Umschalter mit allen drei Optionen (Resultate: 2 Ligen, 2 Teams).
$GLOBALS['wp_options']['svc_settings']['group_switcher'] = 1;
$html = SVC_Shortcodes::club_results( array( 'limit' => 10 ) );
check( 'W2 Umschalter mit Chronologisch/Liga/Team', str_contains( $html, 'data-svc-group="none"' ) && str_contains( $html, 'data-svc-group="league"' ) && str_contains( $html, 'data-svc-group="team"' ) );
check( 'W3 Karten tragen Daten-Attribute', str_contains( $html, 'data-svc-league="' ) && str_contains( $html, 'data-svc-team="Herren 1"' ) );
check( 'W4 Startansicht chronologisch', str_contains( $html, 'data-svc-initial="none"' ) );

// W5: group_by als Startansicht.
$html = SVC_Shortcodes::club_results( array( 'limit' => 10, 'group_by' => 'team' ) );
check( 'W5 group_by wird zur Startansicht', str_contains( $html, 'data-svc-initial="team"' ) && ! str_contains( $html, 'svc-game-group' ) );

// W6: Attribut switcher="0" übersteuert die Einstellung (serverseitige Gruppierung greift wieder).
$html = SVC_Shortcodes::club_results( array( 'limit' => 10, 'group_by' => 'team', 'switcher' => '0' ) );
check( 'W6 switcher="0" übersteuert Einstellung', ! str_contains( $html, 'svc-switcher' ) && str_contains( $html, 'svc-game-group' ) );

// W7: Einstellung aus, Attribut switcher="1" schaltet ein.
$GLOBALS['wp_options']['svc_settings']['group_switcher'] = 0;
$html = SVC_Shortcodes::club_results( array( 'limit' => 10, 'switcher' => '1' ) );
check( 'W7 switcher="1" übersteuert Einstellung', str_contains( $html, 'svc-switcher' ) );

// W8: Nur eine Dimension → nur sinnvolle Buttons (Team-Liste: 1 Team, 2 Ligen → kein Team-Button).
$html = SVC_Shortcodes::games( array( 'team' => '101', 'scope' => 'all', 'switcher' => '1' ) );
check( 'W8 Nur-Liga-Umschalter bei einzelnem Team', str_contains( $html, 'data-svc-group="league"' ) && ! str_contains( $html, 'data-svc-group="team"' ) );

// W9: Nichts zu gruppieren (eine Liga, ein Team) → gar kein Umschalter.
$html = SVC_Shortcodes::results( array( 'team' => '201', 'switcher' => '1' ) );
check( 'W9 Keine Dimension: kein Umschalter', ! str_contains( $html, 'svc-switcher' ) );
$GLOBALS['wp_options']['svc_settings']['group_switcher'] = 0;
